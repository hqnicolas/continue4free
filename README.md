...
- connect the openwebui to the 1340 port
...
````
to list all models: /v1/models
 for more details: /ollama/docs
````
...
docker pull hlohaus789/g4f
docker run \
  -p 8084:8084 -p 1340:1340 -p 7904:7904 \
  --shm-size="2g" \
...
3. **Access the Client:**
   ...
   - To use the included client, navigate to: [http://localhost:8084/chat/](http://localhost:8084/chat/)
   - Or set the API base for your client to: [http://localhost:1340/v1](http://localhost:1340/v1)

4. **(Optional) Provider Login:**
   If required, you can access the container's desktop here: http://localhost:7904/?autoconnect=1&resize=scale&password=secret for provider login purposes.
...
3. **Open GUI**: The app starts a web server with the GUI. Open your favorite browser and navigate to `http://localhost:8084/chat/` to access the application interface.
...
```bash
python -m g4f.cli gui -port 8084 -debug
```
...
**Access with:** http://localhost:1340/v1
