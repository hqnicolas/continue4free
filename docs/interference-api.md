...
**Once running, the API will be accessible at:** `http://localhost:1340/v1`
...
**For text generation:**
```bash
curl -X POST "http://localhost:1340/v1/chat/completions" \
...
1. **url:**
```bash
curl -X POST "http://localhost:1340/v1/images/generate" \
     -H "Content-Type: application/json" \
     -d '{
...
client = OpenAI(
    api_key="",
    base_url="http://localhost:1340/v1"
)
